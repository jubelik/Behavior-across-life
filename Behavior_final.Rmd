...
comp <-read_excel("XXX/Data_Sylvain.xlsx", sheet = "Analysis")
...
comp <- droplevels(comp[!comp$ID == 'EPP24-27',]) #died before 260
comp <- droplevels(comp[!comp$ID == 'EPP24-15',]) #died before 260
comp <- droplevels(comp[!comp$ID == 'EPP24-9',]) #outlier
comp <- comp[-25,] #video of 20 min instead of 30
comp <- droplevels(comp[!comp$ID == 'EPP24-8',]) #died before 260
#S1 for details
...
#graph not kept, but data of all stages put together in Figure 1.
